...
switch lower(BME.Experts.Type)
    case 'mtl'
        %%%%%%%%%input%%%%%%%%%%%%%%%%%%%%%%%%
        Input = BME.Experts.MTLinput{i};
        Target = BME.Experts.MTLtarget{i};
        param = BME.Experts.para{i};
        ...
        gradient = BME.Experts.Alpha*GatingPosterior;%actually it's the prefix of the gradient
        
        task_num = length(category_name);
        Grad = cell([1, task_num]);
        
        for t = 1: task_num
            isSelected = strcmp(category_index, category_name(t));
            selected = gradient(isSelected, :);
            Grad{t} = selected;
        end
